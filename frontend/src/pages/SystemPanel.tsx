        ...
        <div className="h-full overflow-auto p-5 md:p-8">
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1.9fr)_360px]">
                <div className="space-y-4">
                    <section className="rounded-card border border-surface-4 bg-surface-1 p-4 space-y-4">
                        <PageHeader
                            variant="embedded"
                            title="System Panel"
                            description="Local system access with plan-first execution, confirmations, and live results."
                            actions={
                                <div className="rounded-none border border-surface-4 bg-surface-0 px-3 py-1 text-[11px] uppercase tracking-[0.18em] text-text-secondary">
                                    Session {activeSessionId.slice(0, 8)}
                                </div>
                            }
                        />
                        <textarea value={task} onChange={(event) => setTask(event.target.value)} rows={6} className="w-full rounded-card border border-surface-4 bg-surface-0 px-3 py-3 text-sm" />
                        <div className="flex flex-wrap gap-2">
                            <button type="button" onClick={() => setMode('supervised')} className={cn('rounded-btn border px-3 py-2 text-left', mode === 'supervised' ? 'border-accent-blue/40 bg-accent-blue/10' : 'border-surface-4 bg-surface-0')}><p className="text-sm text-text-primary">Supervised</p><p className="mt-1 text-[11px] text-text-secondary">Confirm caution and destructive steps.</p></button>
                            <button type="button" onClick={() => setMode('autonomous')} className={cn('rounded-btn border px-3 py-2 text-left', mode === 'autonomous' ? 'border-accent-blue/40 bg-accent-blue/10' : 'border-surface-4 bg-surface-0')}><p className="text-sm text-text-primary">Autonomous</p><p className="mt-1 text-[11px] text-text-secondary">Only destructive steps require confirmation.</p></button>
                        </div>
                        <div className="flex flex-wrap gap-2">
                            ...
                            <button type="button" onClick={() => { setBrowserOpen(true); void openBrowserAt(browserPath || '~'); }} className="inline-flex items-center gap-2 rounded-btn border border-surface-4 bg-surface-0 px-3 py-2 text-xs text-text-primary"><FolderOpen size={14} />Browse Files</button>
                            <button type="button" onClick={() => void loadDashboard()} className="inline-flex items-center gap-2 rounded-btn border border-surface-4 bg-surface-0 px-3 py-2 text-xs text-text-primary"><Activity size={14} />System Stats</button>
                            <button type="button" onClick={() => setTask('Find Python files in my Documents folder and summarize the important ones.')} className="inline-flex items-center gap-2 rounded-btn border border-surface-4 bg-surface-0 px-3 py-2 text-xs text-text-primary"><Search size={14} />Search Files</button>
                            <button type="button" onClick={() => void handleScreenshot()} className="inline-flex items-center gap-2 rounded-btn border border-surface-4 bg-surface-0 px-3 py-2 text-xs text-text-primary"><Monitor size={14} />Screenshot</button>
                        </div>
                    </section>

                    <section className="rounded-card border border-surface-4 bg-surface-1 p-4 space-y-3">
                        <h2 className="text-base font-semibold text-text-primary">Agent Activity Feed</h2>
                        {plan.length === 0 && <div className="rounded-btn border border-surface-4 bg-surface-0 p-4 text-sm text-text-secondary">Submit a task to generate a plan and stream execution here.</div>}
                        {plan.map((step) => {
                            const state = stepStates[step.step] || { status: 'pending' as StepStatus };
                            const badge = state.status === 'success' ? 'border-accent-green/30 bg-accent-green/10 text-accent-green' : state.status === 'running' ? 'border-accent-blue/30 bg-accent-blue/10 text-accent-blue' : state.status === 'error' ? 'border-accent-red/30 bg-accent-red/10 text-accent-red' : state.status === 'skipped' ? 'border-accent-amber/30 bg-accent-amber/10 text-accent-amber' : 'border-surface-4 bg-surface-0 text-text-secondary';
                            const card = step.is_destructive && state.status === 'pending' ? 'border-accent-amber/30 bg-accent-amber/10' : 'border-surface-4 bg-surface-0';
                            return (
                                <div key={step.step} className={cn('rounded-btn border p-3', card)}>
                                    <div className="flex items-start justify-between gap-3">
                                        ...
                                        <span className={cn('inline-flex items-center gap-1 rounded-none border px-2 py-1 text-[10px] uppercase tracking-wide', badge)}>{state.status === 'success' ? <CheckCircle2 size={14} /> : state.status === 'error' ? <XCircle size={14} /> : state.status === 'running' ? <Activity size={14} /> : <AlertTriangle size={14} />}{state.status}</span>
                                    </div>
                        ...
                        {lastScreenshot?.base64 && <div className="rounded-btn border border-surface-4 bg-surface-0 p-3"><div className="flex items-center justify-between gap-2"><p className="text-sm text-text-primary">Latest Screenshot</p><span className="text-[11px] text-text-secondary">{lastScreenshot.timestamp}</span></div><img src={`data:image/png;base64,${lastScreenshot.base64}`} alt="Latest system screenshot" className="mt-3 max-h-[280px] w-full rounded-btn border border-surface-4 object-contain" /></div>}
                ...
                <aside className="space-y-4">
                    ...
                    <section className="rounded-card border border-surface-4 bg-surface-1 p-4 space-y-3">
                        ...
                        {processes.length === 0 ? <p className="text-xs text-text-secondary">No process data available.</p> : processes.map((proc) => (
                            <div key={proc.pid} className="rounded-btn border border-surface-4 bg-surface-0 p-3">
            ...
            {browserOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
                    <div className="max-h-[90vh] w-full max-w-[min(112rem,calc(100%-2rem))] overflow-hidden rounded-card border border-surface-4 bg-surface-1 shadow-none">
                        <div className="flex items-center justify-between border-b border-surface-4 px-4 py-3"><div><p className="text-sm font-semibold text-text-primary">File Browser</p><p className="text-[11px] text-text-secondary">{browserPath}</p></div><div className="flex gap-2"><button type="button" onClick={() => { const next = browserPath.replace(/[\\/][^\\/]+$/, ''); if (next && next !== browserPath) void openBrowserAt(next); }} className="rounded-btn border border-surface-4 px-3 py-2 text-xs text-text-secondary">Up</button><button type="button" onClick={() => setBrowserOpen(false)} className="rounded-btn border border-surface-4 px-3 py-2 text-xs text-text-secondary">Close</button></div></div>
                        <div className="grid max-h-[calc(90vh-65px)] overflow-y-auto lg:h-[calc(90vh-65px)] lg:grid-cols-[220px_minmax(0,1fr)_360px] lg:grid-rows-[minmax(0,1fr)] lg:overflow-visible">
                            <div className="border-b border-surface-4 bg-surface-0 p-3 overflow-auto lg:border-b-0 lg:border-r"><p className="mb-2 text-[11px] uppercase tracking-wide text-text-secondary">Directories</p>{browserLoading ? <p className="text-xs text-text-secondary">Loading...</p> : directories.map((item) => <button key={item.path} type="button" onClick={() => void openBrowserAt(item.path)} className="mb-2 block w-full rounded-btn border border-surface-4 px-3 py-2 text-left text-sm text-text-primary hover:bg-surface-1">{item.name}</button>)}</div>
                            <div className="border-b border-surface-4 p-3 overflow-auto lg:border-b-0 lg:border-r"><p className="mb-2 text-[11px] uppercase tracking-wide text-text-secondary">Items</p>{browserItems.map((item) => <button key={item.path} type="button" onClick={() => void handleFileSelect(item)} className="mb-2 flex w-full items-center justify-between rounded-btn border border-surface-4 bg-surface-0 px-3 py-2 text-left hover:bg-surface-1"><span className="min-w-0 truncate text-sm text-text-primary">{item.is_dir ? `${item.name}\\` : item.name}</span><span className="ml-3 flex-shrink-0 text-[11px] text-text-secondary">{item.size_human}</span></button>)}</div>
                            <div className="p-3 overflow-auto"><p className="mb-2 text-[11px] uppercase tracking-wide text-text-secondary">Preview</p>{!previewFile ? <p className="text-sm text-text-secondary">Select a file to preview it.</p> : <div className="space-y-3"><div><p className="text-sm font-semibold text-text-primary">{previewFile.name}</p><p className="mt-1 text-[11px] text-text-secondary">{previewFile.path}</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => void openSystemPath(previewFile.path)} className="inline-flex items-center gap-1 rounded-btn border border-surface-4 px-3 py-2 text-xs text-text-primary"><ExternalLink size={14} />Open</button><button type="button" onClick={() => { if (navigator.clipboard) void navigator.clipboard.writeText(previewFile.path); }} className="rounded-btn border border-surface-4 px-3 py-2 text-xs text-text-primary">Copy Path</button><button type="button" onClick={() => { queueChatPrompt(`Analyze this file:\n${previewFile.path}`); navigate('/chat'); }} className="rounded-btn border border-accent-blue/30 px-3 py-2 text-xs text-accent-blue">Ask AI About This File</button></div><pre className="whitespace-pre-wrap p-3 text-[11px] text-text-primary">{previewContent}</pre>{previewTruncated && <p className="text-[11px] text-text-secondary">Preview truncated for readability.</p>}<div className="rounded-btn border border-surface-4 bg-surface-0 p-3 text-[11px] text-text-secondary">{previewFile.size_human} • modified {previewFile.modified}</div></div>}</div>
                        </div>
                    </div>
                </div>
            )}
            ...
            {pendingModal && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-4">
                    <div className="w-full max-w-md rounded-card border border-surface-4 bg-surface-1 p-5">
                        <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1"><p className="text-sm font-semibold text-text-primary">{pendingModal.kind === 'agent' ? 'Confirmation Needed' : 'Kill Process?'}</p><p className="mt-2 text-sm text-text-secondary">{pendingModal.kind === 'agent' ? pendingModal.description : `Terminate ${pendingModal.processName} (PID ${pendingModal.pid})?`}</p>{pendingModal.kind === 'agent' && <span className={cn('mt-3 inline-flex rounded-none border px-2 py-1 text-[10px] uppercase tracking-wide', pendingModal.risk === 'destructive' ? 'border-accent-red/30 bg-accent-red/10 text-accent-red' : 'border-accent-amber/30 bg-accent-amber/10 text-accent-amber')}>{pendingModal.risk}</span>}</div>
                        </div>
                        <div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => void confirmModal(false)} className="rounded-btn border border-surface-4 px-4 py-2 text-sm text-text-secondary">Deny</button><button type="button" onClick={() => void confirmModal(true)} className="rounded-btn border border-accent-green/40 bg-accent-green/10 px-4 py-2 text-sm text-accent-green">Approve</button></div>
                    </div>
                </div>
            )}
        </div>
...
function Metric({ label, value, percent, detail }: { label: string; value: string; percent: number; detail: string }) {
    return (
        <div className="rounded-btn border border-surface-4 bg-surface-0 p-3">
            ...
            <div className="mt-2 h-1.5 overflow-hidden bg-surface-2"><div className="h-full bg-text-primary/40" style={{ width: percentWidth(percent) }} /></div>
