...
const FLOW_NODE_LIBRARY: Record<FlowKind, { title: string; nodeType: string; color: string; description: string }> = {
    trigger: { title: 'Trigger', nodeType: 'jimai.trigger.manual', color: '#155F83', description: 'Entry point.' },
    action: { title: 'Action', nodeType: 'jimai.action.transform', color: '#4D4B49', description: 'Transform payload.' },
    logic: { title: 'Logic', nodeType: 'jimai.logic.condition', color: '#A8561B', description: 'Conditional gate.' },
    ai: { title: 'Local AI', nodeType: 'jimai.ai.ollama', color: '#58593F', description: 'Local Ollama node.' },
    integration: { title: 'Integration', nodeType: 'jimai.integration.http', color: '#350E20', description: 'HTTP integration.' },
    research: { title: 'Research', nodeType: 'jimai.research.search', color: '#800000', description: 'Web research node.' },
};
...
const INITIAL_FLOW_EDGES: Edge[] = [
    { id: 'edge-1', source: 'trigger-1', target: 'ai-1', type: 'smoothstep', animated: true, style: { stroke: '#800000', strokeWidth: 1.5, opacity: 0.7 } },
];
...
function nodeStyle(kind: FlowKind): Record<string, string | number> {
    ...
    return {
        ...
        color: '#1A1918',
        background: '#FFFFFF',
        boxShadow: `0 0 0 1px ${color}18, 0 2px 8px rgba(26,25,24,0.12)`,
        fontSize: '12px',
...
export default function Automation() {
    ...
    const onConnect = useCallback(
        (params: Connection) =>
            setEdges((current) =>
                addEdge({ ...params, type: 'smoothstep', animated: true, style: { stroke: '#800000', strokeWidth: 1.5, opacity: 0.7 } }, current),
            ),
        [setEdges],
    );
