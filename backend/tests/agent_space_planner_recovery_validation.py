"""Purpose: Validate planner failure auto-recovery behavior. Date: 2026-03-10."""
...
async def main_async() -> None:
    ...
    async def fake_execute_subagent(self, *args: Any, **kwargs: Any) -> dict[str, Any]:
        ...
        if role == "planner":
            raise RuntimeError("intentional planner failure for recovery validation")
        if role == "verifier":
            return {"accepted": True, "summary": "verification passed"}
        return {"success": True, "summary": "worker completed"}
    ...
    try:
        ...
        run = await runtime.orchestrator.start_run(
            {
                "objective": "Build an app and include current market research for positioning.",
                "autonomous": False,
                "review_gate": False,
                "allow_shell": False,
                ...
                "subagents": [
                    ...
                    {"id": "worker", "role": "coder", "depends_on": ["planner"], "actions": []},
                    {"id": "verifier", "role": "verifier", "depends_on": ["worker"]},
                ],
            }
        )
        run_id = str(run["id"])
        ...
        print("PLANNER RECOVERY RUN:", {"id": run_id, "status": final.get("status")})
        print(
            "PLANNER RECOVERY EVENTS:",
            {
                "subagent_error": "subagent.error" in event_types,
                "planner_recovered": "planner.recovered" in event_types,
        ...
        assert "planner.recovered" in event_types
        assert "run.completed" in event_types
        print("PLANNER RECOVERY VALIDATION RESULT: PASS")
    finally:
